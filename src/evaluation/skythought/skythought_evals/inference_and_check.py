...
def fetch_response_openai(llm, model_name, max_tokens, temp, top_p, prompt):
    model_name = model_name.replace("openai/", "")
    if "o1" in model_name or "o4" in model_name or "o3" in model_name:
...
def perform_inference_and_check(
    ...
    for temp in temperatures:
        if args.model.startswith("openai"):
            fetch_partial = partial(
                fetch_response_openai, llm, args.model, max_tokens, temp, 0.95
            )
            ...
            sampling_params = SamplingParams(
                n=args.n, max_tokens=max_tokens, temperature=temp, top_p=0.95
            )
            # responses = llm.chat(
            #     messages=conversations, sampling_params=sampling_params, use_tqdm=True, add_generation_prompt=False, continue_final_message=True,
            # )
            print(
                "[DEBUG] Conversations last message role:", conversations[0][-1]["role"]
            )
            if conversations[0][-1]["role"] == "assistant":
                responses = llm.chat(
        ...
        total_correct = 0
        raw_predictions = {idx: [] for idx in range(args.n)}
        raw_answers = []
        total_finish = 0
        completion_tokens = []
        prompt_tokens = []
        ...
        for sample_idx in range(args.n):
            ...
            for idx, response in tqdm(
                enumerate(responses), desc="Processing Responses"
            ):
                if args.model.startswith("openai"):
                    response_str = response.choices[0].message.content.strip()
                else:
                    response_str = response.outputs[sample_idx].text.strip()

                response_entry = handler.update_results(
                    remaining_data[idx], response_str
                )
                ...
                if sample_idx == 0:
                    raw_answers.append(response_entry["answer"])
                ...
                raw_predictions[sample_idx].append(response_entry["pred"])

                if args.model.startswith("openai"):
                    token_usages[idx] = response.usage
                    completion_tokens.append(token_usages[idx].completion_tokens)
                    prompt_tokens.append(token_usages[idx].prompt_tokens)
                    results[problem_key]["token_usages"][str(temp)].append(
                        {
        ...
        majority_vote_acc = 0
        for idx in range(len(raw_answers)):
            votes = [pred[idx] for pred in raw_predictions.values()]
            # voted_raw_prediction = max(set(votes), key=votes.count)
            vote_counts = Counter(votes)
            max_count = max(vote_counts.values())
            top_items = [
                item for item, count in vote_counts.items() if count == max_count
            ]
            for item in top_items:
                if math_equal(item, raw_answers[idx]):
                    majority_vote_acc += 1
                    break
        majority_vote_acc = round(majority_vote_acc / len(raw_answers), 4)
        print(
            json.dumps(
                {
                    "num_sampling": args.n,
                    "acc": acc,
                    f"cons@{args.n}": majority_vote_acc,
                },
                indent=4,
            )
        )
    ...
    token_dict = {
        "completion_tokens": sum(completion_tokens) / args.n,
        "prompt_tokens": sum(prompt_tokens) / args.n,
...
def main():
    ...
    if args.check:
        ...
        llm = (
            OpenAI(base_url="http://localhost:8080/v1/", api_key="-")
            if args.model.startswith("openai")
            else LLM(
                model=args.model,
